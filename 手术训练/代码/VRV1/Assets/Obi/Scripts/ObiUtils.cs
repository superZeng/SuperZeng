...
public static class ObiUtils
{
	...
	public static Vector3 FNorm(this Vector3 v){
		return v/FSqrt(v.sqrMagnitude);
	}

	public static float FLength(this Vector3 v){
		return FSqrt(v.sqrMagnitude);
	}
	...
	*/
	...
	public static float TriangleArea(Vector3 p1, Vector3 p2, Vector3 p3){
		return FSqrt(Vector3.Cross(p2-p1,p3-p1).sqrMagnitude) / 2f;
	}
	 ...
	 */
	public static void EscapePoint(this SphereCollider sc, Vector3 position, float contactOffset, out Vector3 point, out Vector3 normal, out float distance){
		...
		float radius = sc.radius * Mathf.Max(sc.transform.lossyScale.x,
		                                     ...
		                                     sc.transform.lossyScale.z) + Mathf.Max(sc.contactOffset, contactOffset);
		
		float centerToPointMagnitude = FSqrt(position.sqrMagnitude);
	 ...
	 */
	public static void EscapePoint(this CapsuleCollider cc, Vector3 position, float contactOffset, out Vector3 point, out Vector3 normal,out float distance){
		...
		float maxOffset = Mathf.Max(cc.contactOffset, contactOffset);

		switch(cc.direction){
			case 0:{ //X
				radius = cc.radius * Mathf.Max(cc.transform.lossyScale.y,cc.transform.lossyScale.z) + maxOffset;
				height = Mathf.Max(radius,Mathf.Abs(cc.height * 0.5f * cc.transform.lossyScale.x)) + maxOffset;
				d = position.x;
				axisProj = Vector3.right * d;
				cap = Vector3.right * (height - radius);
			}break;
			case 1:{ //Y
				radius = cc.radius * Mathf.Max(cc.transform.lossyScale.x,cc.transform.lossyScale.z) + maxOffset;
				height = Mathf.Max(radius,Mathf.Abs(cc.height * 0.5f * cc.transform.lossyScale.y)) + maxOffset;
				d = position.y;
				axisProj = Vector3.up * d;
				cap = Vector3.up * (height - radius);
			}break;
			case 2:{ //Z
				radius = cc.radius * Mathf.Max(cc.transform.lossyScale.x,cc.transform.lossyScale.y) + maxOffset;
				height = Mathf.Max(radius,Mathf.Abs(cc.height * 0.5f * cc.transform.lossyScale.z)) + maxOffset;
				d = position.z;
		...
		if (d > height - radius){ //one cap
			...
			centerToPointMagnitude = FSqrt(centerToPoint.sqrMagnitude);
	 ...
	 */
	public static void EscapePoint(this BoxCollider bc, Vector3 position, float contactOffset, out Vector3 point, out Vector3 normal, out float distance){
		...
		Vector3 size = Vector3.Scale(bc.size*0.5f,bc.transform.lossyScale) + Vector3.one * Mathf.Max(bc.contactOffset,contactOffset);
		...
		if (distances[0] >= 0 && distances[1] >= 0 && distances[2] >= 0){
			...
			distance = FSqrt(diff.sqrMagnitude);
	 ...
	 */
	public static void EscapePoint(this TerrainCollider tc, Vector3 position, float contactOffset, out Vector3 point, out Vector3 normal, out float distance){
		...
		distance = position.y - tc.terrainData.GetInterpolatedHeight(normalizedPos.x,normalizedPos.y) - Mathf.Max(tc.contactOffset,contactOffset);	
	 ...
	 */
	public static void EscapePoint(this DistanceFieldCollider df, Vector3 position, float contactOffset, out Vector3 point, out Vector3 normal, out float distance){
		...
		distance = df.distanceField.DistanceAndGradientAt(position, out gradient) - Mathf.Max(df.contactOffset,contactOffset) / Mathf.Max(df.transform.lossyScale.x,
																																		  df.transform.lossyScale.y,
																																		  df.transform.lossyScale.z);
