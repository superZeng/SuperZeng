	...
	public IEnumerator Generate() {
		...
		while(queue.Count > 0)
		{
			...
			for (int i = 0; i < 8; i++){
				...
				node.distances[i] = signed ? si.signedDistance : si.unsignedDistance;
				if (highQualityGradient)
					node.gradients[i] = signed ? si.signedVectorToSurface : si.vectorToSurface;
			}
			...
			for (int i = 0; i < 7; i++){
				BIH.SurfaceInfo si = bih.DistanceToSurface(node.bounds.center + Vector3.Scale(node.bounds.extents,errorSamples[i] * 0.5f));
				realDistances[i] = signed ? si.signedDistance : si.unsignedDistance;
			}
