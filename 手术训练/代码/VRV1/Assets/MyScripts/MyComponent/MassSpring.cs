...
public class MassSpring_
{
	...
	public void DeformationCalculate()
	{
		...
		for(int i=0;i<times;i++)
		{
			...
			{
				if(vector_particles[j].velocity.GetSize()!=0)
				{
			...
			for(int j=0;j<vector_particles.Count;j++)
			{
                ...
                if (vector_particles[j].velocity.GetSize() >= 0.2) vector_particles[j].velocity.SetCpr(0.2f, vector_particles[j].velocity.GetDir());
			}
		}
	}
...
public class MassSpring : MonoBehaviour {
	...
	void Start () {
        ...
        obj1 = GameObject.Find("ClampsBody");
    ...
    void OnCollisionStay(Collision collision)
    {
        ...
        //wait until it is safe to enter.
        //mut.WaitOne();
        controlTools ct = obj1.GetComponent<controlTools>();

        Vector3 ve, ve_;
        ve = collision.contacts[0].point;
        ve_ = ct.GetForce() / 1000f;
