...
public sealed class ObiCloth : MonoBehaviour
{
	...
	// Overall behavior:
	[Tooltip("Number of worker threads to spawn for paralelizable tasks. Use 0 to do everything in the main thread.")]
	...
	public ClothInterpolation interpolation = ClothInterpolation.NONE;	/**< Type of physics interpolation.*/
	...
	[HideInInspector] public SpringSet tethers = null; 		 				/**< set of tether springs. */
	...
	public TetherConstraintGroup tetherConstraintsGroup;
	...
	// additional particle state:
	[HideInInspector] public Vector3[] pp_previousPosition = new Vector3[0];	/**< position of particles in the previous simulation step*/
	[HideInInspector] public Vector3[] pp_drawPosition = new Vector3[0];   		/**< position each particle should be drawn at this frame (not the same as particle.position if you use interpolation)*/
	...
	private void GenerateConstraintGroups(){
		...
		if (tetherConstraintsGroup == null)
			tetherConstraintsGroup = new TetherConstraintGroup();
	...
	void FixedUpdate ()
	{
		...
		if ((skinnedMeshRenderer != null && skinnedMeshRenderer.isVisible) || 
			...
			for (int i = 0; i < particles.Count; i++)
				pp_previousPosition[i] = particles[i].position;
	...
	void LateUpdate()
	{
		...
		if (particles != null){
			switch (interpolation){
				...
				case ClothInterpolation.NONE:
					for (int i = 0; i < particles.Count; i++)
						pp_drawPosition[i] = particles[i].position;
					break;
	
				case ClothInterpolation.INTERPOLATE: 
					float alpha = accumulatedTime / Mathf.Max(0.0001f,Time.fixedDeltaTime);
					for (int i = 0; i < particles.Count; i++)
		                    pp_drawPosition[i] = Vector3.Lerp(pp_previousPosition[i],particles[i].position,alpha);
		                break;
	...
	/**
	 * Resizes all per-particle data arrays. TODO: eventually move almost all particle data to arrays for better cache locality.
	 */
	public void ResizeArrays(){
		...
		Array.Resize(ref pp_previousPosition,particles.Count);
		Array.Resize(ref pp_drawPosition,particles.Count);

		if (skinConstraintsGroup != null){
			skinConstraintsGroup.ResizeArrays(particles.Count);
		}
	}
	...
	{		
		...
		stretchConstraintsGroup.ColorizeConstraintGraph(edgeStructure,structuralSprings);

		tethers = new SpringSet();
		pins = new PinSet();
		...
		foreach(ObiClothParticle p in particles)
			pp_previousPosition[p.index] = pp_drawPosition[p.index] = p.position;
			...
			ObiThreads.DoTask(numThreads,(int totalThreads, int threadIndex,object state)=>{
				...
				for (int j = start; j < end; j++){
					...
					if (p.w > 0){
						...
						if (p.asleep && worldVelocity.sqrMagnitude > squaredWakeupVelocity)
							p.WakeUp(minimumWakeTime);
	...
	private void ApplySkinning(){
		...
		if (skinnedMeshRenderer != null){
			...
			for(int i = 0; i < particles.Count; i++){
				ObiClothParticle p = particles[i];
	...
	private void SetupConstraintParameters(){
		if (edgeStructure.IsClosed){
			pressureConstraintsGroup.numThreads = numThreads;
		...
		tetherConstraintsGroup.SetupConstraintParameters(tethers);
		if (IsSkinned)
		...
		pinConstraintsGroup.SetupConstraintParameters(pins);
	}
	...
	private int GetMaxIterations(){
		return Mathf.Max(stretchConstraintsGroup.effectiveIterations,
		                 tetherConstraintsGroup.effectiveIterations,
	...
	public void SimulateStep(float dt){
		...
		// Add external acceleration & forces. First, world movement.
		ApplyWorldSpaceMovement(dt);

		// Then, gravity and aerodynamics:
		...
		int tetherIterationPadding = tetherConstraintsGroup.IterationPadding(maxIter);
		...
		for (int i = 1; i < maxIter; i++){
			...
			if (i % stretchIterationPadding == 0)
				tetherConstraintsGroup.Evaluate(tethers,edgeStructure,particles,numThreads,dt);

			// GROUP 1 ++++++ pins:
			if (i % pinIterationPadding == 0)
				pinConstraintsGroup.Evaluate(pins,edgeStructure,particles,numThreads,dt);

			// GROUP 2 ++++++ bending:
			if (i % bendIterationPadding == 0)
				bendConstraintsGroup.Evaluate(bendingConstraints,edgeStructure,particles,numThreads,dt);

			// GROUP 3 ++++++ pressure:
			if (edgeStructure.IsClosed && i % pressureIterationPadding == 0)
				pressureConstraintsGroup.Evaluate(new List<PressureConstraint>(){pressureConstraint},edgeStructure,particles,0,dt);

			// GROUP 4 ++++++ stretch:
			if (i % stretchIterationPadding == 0)
				stretchConstraintsGroup.Evaluate(structuralSprings,edgeStructure,particles,numThreads,dt);

			// GROUP 5 ++++++ self collisions:
			if (i % selfCollisionIterationPadding == 0)
				selfCollisionConstraintsGroup.Evaluate(scollisions,edgeStructure,particles,numThreads,dt);

			// GROUP 6 ++++++ collisions:
			if (i % collisionIterationPadding == 0)
				collisionConstraintsGroup.Evaluate(collisions,edgeStructure, particles,numThreads,dt);

			// GROUP 7 ++++++ shape matching:
			if (i % shapeMatchingIterationPadding == 0)
				shapeMatchingConstraintsGroup.Evaluate(new List<ShapeMatchingConstraint>(){shapeMatchingConstraint},edgeStructure,particles,0,dt);

            // GROUP 8 ++++++ skin:
			if (IsSkinned && i % skinIterationPadding == 0)
				skinConstraintsGroup.Evaluate(skinConstraints,edgeStructure,particles,numThreads,dt);
                
    	}	
            
		// All groups perform the last evaluation together, regardless of interleaving.
		if (tetherConstraintsGroup.effectiveIterations > 0) tetherConstraintsGroup.Evaluate(tethers,edgeStructure,particles,numThreads,dt);
		if (pinConstraintsGroup.effectiveIterations > 0) pinConstraintsGroup.Evaluate(pins,edgeStructure,particles,numThreads,dt);
		if (bendConstraintsGroup.effectiveIterations > 0) bendConstraintsGroup.Evaluate(bendingConstraints,edgeStructure,particles,numThreads,dt);
		if (pressureConstraintsGroup.effectiveIterations > 0 && edgeStructure.IsClosed) pressureConstraintsGroup.Evaluate(new List<PressureConstraint>(){pressureConstraint},edgeStructure,particles,numThreads,dt);
		if (stretchConstraintsGroup.effectiveIterations > 0) stretchConstraintsGroup.Evaluate(structuralSprings,edgeStructure,particles,numThreads,dt);
		if (selfCollisionConstraintsGroup.effectiveIterations > 0) collisionConstraintsGroup.Evaluate(collisions,edgeStructure, particles,numThreads,dt);
		if (collisionConstraintsGroup.effectiveIterations > 0) collisionConstraintsGroup.Evaluate(collisions,edgeStructure, particles,numThreads,dt);
		if (shapeMatchingConstraintsGroup.effectiveIterations > 0) shapeMatchingConstraintsGroup.Evaluate(new List<ShapeMatchingConstraint>(){shapeMatchingConstraint},edgeStructure,particles,numThreads,dt);
		if (skinnedMeshRenderer != null && skinConstraintsGroup.effectiveIterations > 0) skinConstraintsGroup.Evaluate(skinConstraints,edgeStructure,particles,numThreads,dt);
	...
	public void UpdateGrid(){
		...
		if (grid != null){
			foreach(ObiClothParticle p in particles){
				...
				Vector3 cellIndex = grid.GetParticleCell(transform.TransformPoint(p.predictedPosition));
	...
	public void ResetGeometry(){
		...
		if (!edgeStructure.IsModified){
			...
			foreach (HalfEdge.HEVertex vertex in edgeStructure.heVertices){
				pp_previousPosition[vertex.index] = pp_drawPosition[vertex.index] = particles[vertex.index].position = particles[vertex.index].predictedPosition = sharedVertices[vertex.physicalIDs[0]];
	...
	public void CommitResultsToMesh()
	{
		...
		Profiler.BeginSample("Commit vertices");
		...
		if (skinnedMeshRenderer != null){ //Update skinned mesh:
			...
			for(int i = 0; i < edgeStructure.heVertices.Count; i++){
				...
				Vector3 position = pp_drawPosition[vertex.index];

				for(int j = 0; j < vertex.physicalIDs.Count; j++){
					isClothVertex[vertex.physicalIDs[j]] = true;
					vertices[vertex.physicalIDs[j]] = skinnedMeshRenderer.rootBone.InverseTransformPoint(transform.TransformPoint(position));
				}
			...
			for(int i = 0; i < edgeStructure.heVertices.Count; i++){
				...
				Vector3 position = pp_drawPosition[vertex.index];
				for(int j = 0; j < vertex.physicalIDs.Count; j++){
					vertices[vertex.physicalIDs[j]] = position;
				}
			}
	...
	public void TearSpring(DistanceConstraint sp,Vector3[] vertices){
		...
		if (edgeStructure.SplitVertex(edgeStructure.heVertices[splitIndex],new Plane((v2-v1).normalized,v1),out newVertex,out vertices,out removedEdges,out addedEdges, out oldNewEdges))
		{
			...
			ObiClothParticle newParticle = new ObiClothParticle(particles.Count,particles[splitIndex].areaContribution,particles[splitIndex].mass,vertices[newVertex.physicalIDs[0]],Vector3.zero);
			...
			// Resize arrays to include new particles, and copy values from split particle:
			ResizeArrays();
			pp_drawPosition[newParticle.index] = pp_drawPosition[splitIndex];
			pp_previousPosition[newParticle.index] = pp_previousPosition[splitIndex];
		}
	...
	public void Optimize(){
		...
		foreach (int index in particlesToOptimize){
			...
			if (index < skinConstraints.Count)
		...
		for(int i = 0; i < particles.Count; i++){
			pp_drawPosition[particles[i].index] = pp_drawPosition[i];
			pp_previousPosition[particles[i].index] = pp_previousPosition[i];
			particles[i].index = i;
		}
		...
		foreach(SkinConstraint s in skinConstraints){
			skinConstraintsGroup.pp_backstop[oldParticles[s.pIndex].index] = skinConstraintsGroup.pp_backstop[s.pIndex];
			skinConstraintsGroup.pp_radius[oldParticles[s.pIndex].index] = skinConstraintsGroup.pp_radius[s.pIndex];
