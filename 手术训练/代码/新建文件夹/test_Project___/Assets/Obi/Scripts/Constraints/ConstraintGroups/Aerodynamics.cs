...
public class Aerodynamics
{
	...
	public void ApplyAerodynamicsToParticle(ObiClothParticle p, Vector3 normal, Vector3 localSpaceWind, float dt){
		...
		if (enabled){
			...
			Vector3 relVelocityNorm = relVelocity / ObiUtils.FSqrt(relVelSqrMag);	//direction of relative velocity.
			
			// Calculate surface normal:
			Vector3 surfNormal = normal.FNorm() * Mathf.Sign(Vector3.Dot(normal,relVelocityNorm));
			...
			float dotNRV = Vector3.Dot(surfNormal,relVelocityNorm);
			...
			Vector3 fLift = liftMagnitude * dotNRV * Vector3.Cross(Vector3.Cross(surfNormal,relVelocityNorm),relVelocityNorm).FNorm();
