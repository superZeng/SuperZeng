...
public class ObiActor : MonoBehaviour
{
	...
	[HideInInspector] public float minimumContactOffset = 0.1f;
	...
	public Bounds bounds{
		get{
			...
			if (dfCollider != null){
				...
				_bounds.Expand((dfCollider.contactOffset + minimumContactOffset) * 2);
			}

			if (colliders.Length > 0){

				_bounds = colliders[0].bounds;
				_bounds.Expand((colliders[0].contactOffset + minimumContactOffset) * 2);

				for (int i = 1; i < colliders.Length; i++){
					_bounds.Encapsulate(colliders[i].bounds);
					_bounds.Expand((colliders[i].contactOffset + minimumContactOffset) * 2);
				}
	...
	public void Initialize(ObiWorld world){
		...
		if (this.world != null){
			...
			for(int i = 0; i < actors.Length;i++){	
				if (actors[i] != this && (actors[i].world == this.world || actors[i].world == null))
